  ...
  const renderContent = () => {
    switch (currentView) {
      ...
      case AppView.COMMITS:
        if (!selectedRepoId) return <RepositoryView onSelectRepo={handleRepoSelect} />;
        return <CommitHistoryView repoId={selectedRepoId} onBack={() => setCurrentView(AppView.REPOSITORIES)} />;
    ...
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans selection:bg-emerald-500/30">
      ...
      <div className="lg:hidden p-4 border-b border-slate-800 flex items-center justify-between bg-slate-900">
        ...
        <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="text-slate-400">
           {isMobileMenuOpen ? <Icons.Error size={24}/> : <div className="space-y-1.5">
      ...
      {isMobileMenuOpen && (
        <div className="lg:hidden fixed inset-0 z-40 bg-slate-900 pt-20 px-4">
             <nav className="space-y-4">
               {[AppView.DASHBOARD, AppView.REPOSITORIES, AppView.SCANNER, AppView.ARCHITECTURE].map(view => (
                 <button 
                   key={view}
                   onClick={() => {
                     setCurrentView(view);
               ...
               <button onClick={handleLogout} className="block w-full text-left py-3 px-4 rounded-lg bg-red-900/20 text-red-400 font-medium mt-4">
                 Sign Out
               </button>
             </nav>
        </div>
