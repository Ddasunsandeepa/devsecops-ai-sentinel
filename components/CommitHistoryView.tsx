...
export const CommitHistoryView: React.FC<CommitHistoryViewProps> = ({
  ...
  useEffect(() => {
    fetch(`http://localhost:3001/api/repos/${repoId}/commits`)
      .then((r) => r.json())
      .then(setCommits);
  }, [repoId]);
  ...
  const handleAnalyze = async (commitId: string, code: string) => {
    ...
    setScanResult(null);
    try {
      const result = await fetch(`http://localhost:3001/api/scan/${commitId}`, {
        method: "POST",
      });
        ...
        <div className="flex justify-between items-start">
          <div>
            <h2 className="text-2xl font-bold text-white flex items-center gap-3">
              <span className="font-mono bg-slate-800 px-2 py-1 rounded text-lg">
                {selectedCommitId}
              </span>
              <span>Scan Report</span>
            </h2>
            <p className="text-slate-400 mt-1">
              Detailed AI analysis of security flaws.
            </p>
          </div>
          {scanResult && (
            <div
            ...
            >
              <span className="text-xs font-bold uppercase block text-center">
                Risk Score
              </span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 flex-1">
            <div className="space-y-4">
              ...
              <div className="bg-slate-950 border border-slate-800 rounded-lg p-4 font-mono text-xs text-slate-300 overflow-auto max-h-[600px] whitespace-pre-wrap">
                {
                  <pre>
                    {scanResult?.source || "Source will appear after scan"}
                  </pre>
                }
              </div>
            </div>
    ...
    <div className="space-y-6 animate-fade-in pb-10">
      ...
      <div>
        <h2 className="text-2xl font-bold text-white">
          Commit History: {repoId}
        </h2>
        <p className="text-slate-400">
          Select a commit to view security analysis details.
        </p>
      </div>
            ...
            {commits.length === 0 ? (
              <tr>
                <td colSpan={5} className="p-8 text-center text-slate-500">
                  No commits found for this repository mock.
                </td>
              </tr>
            ) : (
              commits.map((commit) => (
                <tr
                  key={commit.hash.slice(0, 7)}
                  className="hover:bg-slate-800/50 transition-colors group"
                >
                  <td className="p-4 font-mono text-sm text-emerald-400">
